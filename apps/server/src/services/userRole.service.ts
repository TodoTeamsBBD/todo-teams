...
export const findUserRoleIfExists = (userId: string, roleId: number, teamId?: number) => prisma.user_roles.findFirst({
    where: {
        user_id: userId,
        role_id: roleId, 
        team_id: teamId
...
export const createUserRole = (userId: string, teamId: number, roleId: number) => prisma.user_roles.create({
    user_id: userId,
    team_id: teamId,
    role_id: roleId
});
