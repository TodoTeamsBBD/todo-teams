...
export const getToDo = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (isNaN(id)) {
    return res.status(400).json({ "message": "Todo id must be a valid number" });
  }
...
export const getToDosForTeam = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (isNaN(id)) {
    return res.status(400).json({ "message": "Team id must be a valid number" });
  }
...
export const create = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!title || !description || !assignedTo || !isUuid(assignedTo) || isNaN(teamId)) {
    return res.status(400).json({ "message": "Valid title, description, assigned member, and team id is required" });
  }
  ...
  if (userRole.role_id == rolesEnum.TeamMember) {
    if (userRole.user_id != assignedUser.id) {
      return res.status(403).json({ "message": "You do not have permission to create a todo assigned to another member" });
    }
  }
  else {
    ...
    if (!assignedUserRoleCheck) {
      ...
      if (!assignedUserRoleCheck)
        return res.status(403).json({ "message": "Cannot assign a task to a member not in the team" });
    }
  }
...
export const update = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (userRole.role_id == rolesEnum.TeamMember) {
    if (userRole.user_id != todoToUpdate.assigned_to) {
      return res.status(403).json({ "message": "You do not have permission to update a todo assigned to another member" });
    }

    if (assignedTo) {
      if (assignedTo != userRole.user_id) {
        return res.status(403).json({ "message": "You do not have permission to assign a todo to another member" });
      }
    }
  }
  ...
  if (assignedTo) {
    if (!isUuid(assignedTo)) {
      return res.status(400).json({ "message": "Please enter a valid assigned to" })
    }
...
export const remove = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (userRole.role_id == rolesEnum.TeamMember) {
    if (userRole.user_id != todoToDelete.assigned_to) {
      return res.status(403).json({ "message": "You do not have permission to delete a todo assigned to another member" });
    }
  }
